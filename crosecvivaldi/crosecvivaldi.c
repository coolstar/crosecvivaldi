...
static NTSTATUS send_ec_command(
	...
	size_t inSize)
{
	PCROSEC_COMMAND msg = ExAllocatePoolZero(NonPagedPool, sizeof(CROSEC_COMMAND*) + max(outSize, inSize), CROSECVIVALDI_POOL_TAG);
	if (!msg) {
...
static NTSTATUS GetArrayProperty(
	...
	ACPI_GET_DEVICE_SPECIFIC_DATA* inputBuffer = ExAllocatePoolZero(NonPagedPool, inputBufferLen, CROSECVIVALDI_POOL_TAG);
	if (!inputBuffer) {
		goto Exit;
	}
...
--*/
{
	...
	CrosEcVivaldiCompleteIdleIrp(pDevice);
...
)
{
	...
	PCROSECVIVALDI_CONTEXT deviceContext;
...
NTSTATUS
CrosEcVivaldiProcessIdleRequest(
	IN PCROSECVIVALDI_CONTEXT pDevice,
...
)
{
	...
	*Complete = TRUE;
...

	{
		...
		WDF_WORKITEM_CONFIG_INIT(&workitemConfig, CrosEcVivaldiIdleIrpWorkItem);
...
)
{
	...
	case IOCTL_HID_SEND_IDLE_NOTIFICATION_REQUEST:
		...
		status = CrosEcVivaldiProcessIdleRequest(devContext, Request, &completeRequest);
		break;
